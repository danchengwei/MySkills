 ...
 * 格式化日期显示
 */
function formatDateRange(days, startDate, endDate) {
...
async function installAliyunCLI() {
...
// ==================== EMAS API 调用 ====================
async function callEMASAPI(action, params, config) {
...
// ==================== 查询函数 ====================
async function getIssues(appKey, bizModule, startDate, endDate, config, pageSize = '20', pageIndex = '1', orderBy = 'ErrorCount', orderType = 'desc', version, crashType, lagType) {
    ...
    if (crashType && result.Model?.Items) {
        ...
        // 筛选符合条件的崩溃
        result.Model.Items = allItems.filter((item) => {
 ...
 * 获取崩溃详情
 */
async function getIssueDetail(appKey, bizModule, digestHash, config) {
...
// ==================== 通用分析命令 ====================
function createAnalyzeCommand(type, analysisType) {
    return program
        .command(`${type}`)
        ...
        try {
            ...
            const result = await getIssues(appKey, analysisType.module, start, end, config, pageSize, pageIndex, options.orderBy || 'ErrorCount', options.orderType || 'desc', options.appVersion, options.crashType, lagType);
...
program
    ...
    const config = loadConfig();
    if (options.ak)
        config.accessKeyId = options.ak;
    if (options.sk)
        config.accessKeySecret = options.sk;
    if (options.region)
    ...
    if (options.consoleUrl)
        config.consoleUrlTemplate = options.consoleUrl;
    saveConfig(config);
...
program
    ...
    for (const page of pages) {
        ...
        try {
            ...
            const result = await getIssues(appKey, analysisType.module, start, end, config, options.pageSize.toString(), page.toString(), 'ErrorCount', 'desc', options.appVersion, options.crashType, lagType);
...
program
    ...
    for (const weekRange of weekRanges) {
        ...
        // 获取 Java Crash 数据
        try {
            const javaResult = await getIssues(appKey, 'crash', weekRange.start, weekRange.end, config, '10', '1', 'ErrorCount', 'desc', options.appVersion, 'javaCrash');
        ...
        // 获取 Native Crash 数据
        try {
            const nativeResult = await getIssues(appKey, 'crash', weekRange.start, weekRange.end, config, '10', '1', 'ErrorCount', 'desc', options.appVersion, 'nativeCrash');
 ...
 * 在项目源码中搜索相关文件
 */
async function searchSourceFile(projectPath, className) {
    try {
        // 将类名转换为文件路径
        ...
        const { stdout } = await execAsync(`find "${projectPath}" -name "*.java" -type f 2>/dev/null | xargs grep -l "class ${simpleName}" 2>/dev/null | head -1`, { timeout: 10000 });
 ...
 * 读取并显示源码片段
 */
async function readSourceSnippet(filePath, lineNumber, context = 5) {
    try {
 ...
 * 生成代码修改建议
 */
function generateFixSuggestion(errorType, stack, className) {
    const result = {
        reason: '',
        suggestion: '',
        codeDiff: ''
 ...
 */
async function getGitBlameInfo(projectPath, className, centerLine, contextLines = 8) {
    const result = {
        isGitIgnored: false,
        submodule: '',
        blameLines: []
    ...
    try {
        ...
        if (!actualFile) {
            // 尝试查找
            try {
                const { stdout } = await execAsync(`find "${projectPath}" -name "${className.split('.').pop()}.java" -type f 2>/dev/null | head -1`);
        ...
        // 检查是否被 .gitignore 忽略
        try {
            const { stdout: ignoreStdout } = await execAsync(`cd "${projectPath}" && git check-ignore -v "${relativePath}" 2>/dev/null`, { timeout: 5000 });
            if (ignoreStdout.includes('.gitignore') || ignoreStdout.includes('in repo')) {
                ...
                for (let i = 1; i < parts.length; i++) {
                    ...
                    try {
                        const { stdout: submoduleStdout } = await execAsync(`cd "${projectPath}" && git submodule status "${subpath}" 2>/dev/null`, { timeout: 5000 });
        ...
        try {
            ...
            const { stdout: blameOutput } = await execAsync(`cd "${projectPath}" && git blame --line-porcelain -L ${startLine},${endLine} -- "${relativePath}" 2>/dev/null`, { timeout: 15000 });
            // 解析 blame 输出
 ...
 * 从 Git 获取文件的最近修改信息
 */
async function getGitFileInfo(projectPath, className, lineNumber = '') {
    const result = {
        author: '',
        email: '',
        lastCommit: '',
    ...
    try {
        ...
        // 最近修改者
        try {
            const { stdout: logStdout } = await execAsync(`cd "${projectPath}" && git log -1 --format="%an|%ae|%s|%ai" -- "${relativePath}"`, { timeout: 10000 });
        ...
        if (lineNumber && result.author) {
            try {
                ...
                const { stdout: blameStdout } = await execAsync(`cd "${projectPath}" && git blame -L ${lineNum},${lineNum} -- "${relativePath}" 2>/dev/null | head -1`, { timeout: 10000 });
 ...
 * 智能分析单条崩溃
 */
async function analyzeCrash(crash, projectPath, bizModule, config, appKey, startDate, endDate, index = 1) {
    // 捕获控制台输出
    let reportContent = '';
    ...
    if (crashDetail?.Model?.VersionDistribution) {
        ...
        const totalVersionCrashes = versionDist.reduce((sum, item) => sum + (item.Count || 0), 0);
        // 按崩溃次数排序
        versionDist.sort((a, b) => (b.Count || 0) - (a.Count || 0));
    ...
    if (!isPerformanceAnalysis && FirstVersion) {
        ...
        if (crashDetail?.Model?.VersionDistribution) {
            const versionDist = crashDetail.Model.VersionDistribution;
            const versions = versionDist.map((v) => v.Version).filter(Boolean);
    ...
    if (isPerformanceAnalysis) {
        ...
        if (crashDetail?.Model?.OsDistribution) {
            ...
            const totalOsCrashes = osDist.reduce((sum, item) => sum + (item.Count || 0), 0);
            osDist.sort((a, b) => (b.Count || 0) - (a.Count || 0));
        ...
        if (crashDetail?.Model?.DeviceDistribution) {
            ...
            const totalDeviceCrashes = deviceDist.reduce((sum, item) => sum + (item.Count || 0), 0);
            // 按崩溃次数排序，只显示前5个
            deviceDist.sort((a, b) => (b.Count || 0) - (a.Count || 0));
            const topDevices = deviceDist.slice(0, 5);
            const otherCount = deviceDist.slice(5).reduce((sum, item) => sum + (item.Count || 0), 0);
        ...
        if (crashDetail?.Model?.BrandDistribution) {
            ...
            const totalBrandCrashes = brandDist.reduce((sum, item) => sum + (item.Count || 0), 0);
            // 按崩溃次数排序
            brandDist.sort((a, b) => (b.Count || 0) - (a.Count || 0));
        ...
        if (projectPath && fs.existsSync(projectPath)) {
            if (stackInfo.appInfo.className) {
                ...
                if (!sourceFile) {
                    ...
                    // 需要进行 Git Blame
                    // 获取 Git blame 信息
                    const blameInfo = await getGitBlameInfo(projectPath, stackInfo.appInfo.className, parseInt(stackInfo.appInfo.lineNumber) || 100, 8);
...
// ==================== 辅助函数 ====================
// 获取崩溃类型标签
function getCrashTag(type, bizModule) {
    if (bizModule === 'lag') {
...
// 格式化堆栈信息
function formatStack(stack, maxLines = 15) {
    if (!stack)
        return '-';
    const lines = stack.split('\n').filter(l => l.trim());
...
// 详细输出表格
function displayIssuesTable(data, type, pageIndex = '1', pageSize = '10') {
